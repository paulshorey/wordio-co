...
import { v1_domain_suggestions, v1_whois } from "src/domains/http/v1";
...
export function RX__domain_availability_queue(domain_str) {
  return (dispatch) => {
    /*
     * debounce this
     */
     ...
     */
    dispatch({
      type: "RX__domain_availability_queue",
      data: domain_str
    });
  };
}
...
export function RX__search_thesaurus() {
  return (dispatch, getState) => {
    ...
    v1_get_word("word", params).then(function (row = {}) {
      setTimeout(function () {
        RX__search_thesaurus__in_progress = false;
      }, 1000);
